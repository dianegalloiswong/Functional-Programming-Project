...
let rec incr_from_i_and_Cid i = function
  ...
  | _ -> -1
(* Determine whether [ics] is of the form [(1, Cid); ...; (n, Cid)] with [l] of length [n], in which case [Crecord ics] applied to [l] is equivalent to [Cid]. *)
let crecord_equiv_cid ls ics = (incr_from_i_and_Cid 0 ics) = (List.length ls)

(* Return whether [c] applied to [l] is equivalent to [Cid] with [c] itself distinct from [Cid]. The last condition avoids an infinite loop in [optim]. *)
(*let equiv_Cid l c = match c with
  | Cfun(Cid, Cid) -> true
  | Crecord ics when crecord_equiv_cid l ics -> true
  | _ -> false*)
...
let rec optim_coerce l c = match l, c with
  ...
  | Labstr(x, le), Cfun(c1, c2) ->
	  let l1 = 
	    if c1 = Cid (*|| equiv_Cid (Lvar x) c1*) then le
